...
def set_up():
    ...
    assist_acc_head_dir = resolve_acc_head_path(assistant_checkpoint, checkpoint)

    tokenizer = AutoTokenizer.from_pretrained(checkpoint)

    assistant_model = AutoModelForCausalLM.from_pretrained(
        assistant_checkpoint, torch_dtype=torch.bfloat16, device_map="cuda:0"
    )
    assist_acc_head = AcceptancePredictionHead.from_pretrained(assist_acc_head_dir).to(
        "cuda:0"
    )
...
def main(prompt):
    ...
    outputs, mismatched_tokens, LM_call = my_generate(
        model=model,
        **inputs,
