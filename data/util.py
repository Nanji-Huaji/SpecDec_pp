    ...
    for name, module in fp8_modules:
        ...
        with torch.no_grad():
            ...
            if s_inv.ndim == 2 and s_inv.shape[0] < w_fp8.shape[0]:
                ...
                w_reshaped = w_fp8.view(s_inv.shape[0], h_block, s_inv.shape[1], w_block)
                bf16_weight = (w_reshaped * s_inv.view(s_inv.shape[0], 1, s_inv.shape[1], 1)).reshape(w_fp8.shape)
            else:
